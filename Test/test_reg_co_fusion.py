...
def hyper_args():
    ...
    # TNO dataset
    # parser.add_argument('--it', default='../dataset/raw/ctest/TNO/it_edge', type=pathlib.Path)
    # parser.add_argument('--ir', default='../dataset/raw/ctest/TNO/ir_w', type=pathlib.Path)
    # parser.add_argument('--vi', default='../dataset/raw/ctest/TNO/vi', type=pathlib.Path)
    # parser.add_argument('--disp', default='../dataset/raw/ctest/TNO/disp', type=pathlib.Path)

    # M3FD dataset
    # parser.add_argument('--it', default='../dataset/raw/ctest/M3FD/it_edge', type=pathlib.Path)
    ...
    # parser.add_argument('--disp', default='../dataset/raw/ctest/M3FD/disp', type=pathlib.Path)

    # NIR dataset
    parser.add_argument('--it', default='../dataset/raw/ctest/NIR-S/it_edge', type=pathlib.Path)
    parser.add_argument('--ir', default='../dataset/raw/ctest/NIR-S/ir_warp', type=pathlib.Path)
    parser.add_argument('--vi', default='../dataset/raw/ctest/NIR-S/vi', type=pathlib.Path)
    parser.add_argument('--disp', default='../dataset/raw/ctest/NIR-S/disp', type=pathlib.Path)

    # checkpoint and save path
    parser.add_argument('--ckpt_reg', default='../cache/Joint_reg_fusion/NIR/230224_joint_DUSE_reg_co_fus_SA/reg_0740.pth', help='weight checkpoint', type=pathlib.Path) # weight/default.pth
    parser.add_argument('--dst_reg',  default='../results_NIR/Reg+Fusion/230224_joint_DUSE_reg_co_fus_SA/reg/', help='fuse image save folder', type=pathlib.Path)

    parser.add_argument('--ckpt_fus', default='../cache/Joint_reg_fusion/NIR/230224_joint_DUSE_reg_co_fus_SA/fus_0740.pth', help='weight checkpoint', type=pathlib.Path)  # weight/default.pth
    parser.add_argument('--dst_fus',  default='../results_NIR/Reg+Fusion/230224_joint_DUSE_reg_co_fus_SA/fus/',help='fuse image save folder', type=pathlib.Path)
    # setup
...
def main(args):
    ...
    RegNet  = DeformableDuSENet().to(device)
    # RegNet = PCNet().to(device)
