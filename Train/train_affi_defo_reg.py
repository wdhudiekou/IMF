...
# Training settings
    ...
    parser.add_argument("--ckpt", default="../cache/Reg_only/RoadScene/checkpoint_path/", type=str, help="path to pretrained model (default: none)")
...
def main(args, visdom):
    ...
    for epoch in range(args.start_epoch, args.nEpochs + 1):
        train(training_data_loader, optimizer, net, criterion, epoch)
        if epoch % 20 == 0:
            save_checkpoint(net, epoch, cache)

def get_random_affine_coeff():
    '''
    define coefficent for affine transformation
    '''
    degree = random.uniform(0, 10)
    translate = random.uniform(0.05, 0.20)
...
def get_random_deformable_coeff():
    '''
        define coefficent for deformable transformation
        '''
    kernel = random.randrange(63, 101, 2)
    sigma = random.randint(16, 32)
    alpha = random.uniform(1.0, 1.0)
    return kernel, sigma, alpha

def train(training_data_loader, optimizer, net, criterion, epoch):
