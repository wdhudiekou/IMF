...
class RegData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        assert ir_path.name.split('.')[0] == it_path.name.split('.')[0], f"Mismatch ir:{ir_path.name} vi:{it_path.name}."
...
class RegRealData(torch.utils.data.Dataset):
    """
    Load dataset with infrared folder path and visible folder path
    """
    ...
    def __init__(self, ir_folder: pathlib.Path, it_folder: pathlib.Path):
        super(RegRealData, self).__init__()
